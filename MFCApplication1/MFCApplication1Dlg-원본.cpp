...
typedef struct receiveData {											//roi 영역 txt 파일을 읽어 저장
	...
	bool isRed = false;
	int cars;															//주차 차량수
	string numbers[3] = { "x", "x", "x" };								//번호 판별 결과 저장
}receiveData;
...
typedef struct plateData {												//찾은 번호판 영역좌표 및 기타정보
	...
	string panicName;
	string dvrIP;
	string plateNum[3] = { "x", "x", "x" };
	int cameraNum;
	int x[3], y[3], w[3], h[3];											//찾은 번호판 좌표
	int cPlate = 0;														//찾은 번호판 개수
	bool isRed = false;													//사진에서 만차 판별 결과 ( c# UI 에서 2장 모두 red 일때 red로 변경)
}plateData;
...
int getFileFromHttp(string pszUrl, string pszFile);							//이미지 다운로드
...
void getHttpPath()
{
	...
	while (getline(in, line))			//한라인씩 읽어서 비동기 작업으로 다운로드 처리
	{
		try {
			...
			auto th = async(launch::async, getFileFromHttp, url, path);		//path = 저장될 파일 명
...
int getFileFromHttp(string pszUrl, string pszFile)
{
	...
	try
	{
			...
			0)) != NULL)                // Options
		{
				...
				0)) != NULL)                 // Context
			{
				...
				// 다운로드할 파일 만들기
				if ((fp = fopen(pszFile.c_str(), "wb")) != NULL)
				{
...
Mat concatImg(int index)
{
	...
	//불러온 이미지 1/4 크기로 변경
	string imgPath = "C:\\Program Files (x86)\\Twowinscom\\Parking Guidance\\downImage\\";

	string p, p1, p2, p3, p4;
	thread t[4], t1, t2, t3, t4;
	for (int i = 0; i < size; i++)
	{
		p = imgPath + readList.at(index + i).fileName + ".jpg";
		t[i] = thread([](string path, Mat* img)
		{
			try
			{
				resize(imread(path), *img, cv::Size(960, 540));
			}
			catch (std::exception e) {
				*img = default_image;
				writeErrorMessage(to_string(__LINE__) + " resize image error  :" + (string)e.what());
			}
			catch (cv::Exception& e) //opencv 예외
			{
				writeErrorMessage(to_string(__LINE__) + " resize opencv error :" + (string)e.what());
			}
		}, p, &imgArray[i]);
	}
	...
	try
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			{
			case 0:
				imgArray[i].copyTo(resultMat(cv::Rect(0, 0, 960, 540)));
				break;
			case 1:
				imgArray[i].copyTo(resultMat(cv::Rect(960, 0, 960, 540)));
				break;
			case 2:
				imgArray[i].copyTo(resultMat(cv::Rect(0, 540, 960, 540)));
				break;
			case 3:
				imgArray[i].copyTo(resultMat(cv::Rect(960, 540, 960, 540)));
				break;
			}
		}
	}
		...
		while (true)
		{
			...
			while (index < readList.size())
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					for (int j = 0; j < 3; j++)
					{
						int roi_x = readList.at(count).x[j] / 2;		//분석된 좌표는 resize된좌표, roi 좌표는 원래 좌표이므로 좌표 비율 맞춤
						int roi_y = readList.at(count).y[j] / 2;		//가로 세로 1/2 씩 전체 크기 1/4 
						...
						for (bbox_t box : result_vec)
						{
							...
							{
								if (box.w < 650 && box.h < 450 && box.w > 150 && box.h > 150)	// 객체가 너무 작거나 큰경우 차가 아니므로 필터링
								{
									int left = box.x;
									int top = box.y;
					...
					readList.at(count).cars = cars;					//리스트에 차량 개수 저장
					if (cars == readList.at(count).areas)			//차 개수 =  roi 위치 개수 -> Red
						readList.at(count).isRed = true;
